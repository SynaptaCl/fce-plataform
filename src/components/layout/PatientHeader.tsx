...
export function PatientHeader({ patient, hasConsent, patientId }: PatientHeaderProps) {
  ...
  // Build secondary info items, skipping null/undefined silently
  const infoItems: string[] = [];
  if (patient.rut) infoItems.push("__RUT__"); // placeholder replaced inline
  if (age !== null) infoItems.push(`${age} años`);
  if (sexDisplay) infoItems.push(sexDisplay);
  if (previsionLabel) infoItems.push(previsionLabel);
  if (patient.ocupacion) infoItems.push(patient.ocupacion);
