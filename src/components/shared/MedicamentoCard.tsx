...
export function MedicamentoCard({ medicamento, onUpdate, onRemove }: Props) {
  ...
  return (
    <div className="rounded-lg border" style={{ borderColor: "var(--kp-border)" }}>
      <div className="flex items-center justify-between px-3 py-2">
        ...
        <div className="flex items-center gap-1 ml-2">
          <button onClick={() => setExpanded(!expanded)} className="p-1 rounded hover:bg-gray-100">
            {expanded
              ? <ChevronUp className="size-4" style={{ color: "var(--ink-3)" }} />
              : <ChevronDown className="size-4" style={{ color: "var(--ink-3)" }} />}
          </button>
      ...
      {expanded && (
        <div className="border-t px-3 py-3" style={{ borderColor: "var(--kp-border)" }}>
