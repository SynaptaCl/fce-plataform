...
interface Props {
  ...
  onModoChange: (m: "impresa" | "canvas") => void;
  onFirmaChange: (dataUrl: string | null) => void;
}

/**
 * Panel de firma para órdenes de examen.
 *
 * Modo "impresa": el PDF se imprime para firma manual del profesional.
 * Modo "canvas":  el profesional dibuja su firma directamente en pantalla.
 */
export function OrdenExamenFirmaPanel({
  modoFirma,
  onModoChange,
  onFirmaChange,
}: Props) {
    ...
    <div className="space-y-3">
      ...
      <div className="flex gap-2">
        {(["impresa", "canvas"] as const).map((m) => (
          ...
          >
            {m === "impresa" ? "Firma impresa" : "Firma digital (canvas)"}
          </button>
        ))}
      </div>
